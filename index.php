<!DOCTYPE html>
<html>
<head>
	<title>Airport Storage & Garages</title>
	<meta charset="utf-8">
	<meta description="Storage and large garages located in Newport News, VA">
	<link rel="stylesheet" href="assets/css/style.css" />
	<!--bower:css -->
	<!--endbower-->
</head>
<body class="nothome">
	<div class="container">
		<header class="header">
			<div class="wrap row">
				<div class="logo col-md-4">
					<img src="assets/img/logo.png" alt="airport storage and garages logo" />
				</div>
				<div class="contact col-md-8">
					<a href="tel:+17578819203" class="phone">(757) 881-9203</a>
					<p><span class="glyphicon glyphicon-map-marker"></span>599 Choptank Lane • Newport News, VA</p>
				</div>
			</div><!-- /.wrap -->
		</header>
		<nav>
			<div class="wrap">
				<ul id="main-menu" class="nav nav-pills">
					<li class="active"><a href="#" class="active">Home</a></li>
					<li><a href="#">Storage Suggestions</a></li>
					<li><a href="#">Frequently Asked Questions</a></li>
					<li><a href="#">Reserve Storage</a></li>
				</ul>
			</div>
		</nav>
		<main>
			<h1>Storage Units and Garages serving the Virginia Peninsula</h4>
			<div class="primary row">
				<section class="col-md-4">
					<ul>
						<li>Lowest price per square foot in the area</li>
						<li>Huge storage garages with 14’ ceilings</li>
						<li>Brand new facility with great lighting and security cameras</li>
						<li>Ground level storage units with drive-up access</li>
						<li>10 foot doors that trigger automatic interior lighting when opened</li>
					</ul>
				</section>
				<section class="col-md-8 jumbotron">
					Photos
				</section>
			</div>
			<aside class="business-info">
				<div class="row">
					<div class="col-md-4">
					  	<section>
					  	<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3181.5483461076406!2d-76.51030538458186!3d37.115867357565996!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x89b0792b887fc521%3A0xd770c359549f7fba!2s599+Choptank+Ln%2C+Newport+News%2C+VA+23602!5e0!3m2!1sen!2sus!4v1499716440262" width="100%" height="100%" frameborder="0" style="border:0" allowfullscreen></iframe>
					  	<a class="btn-default btn" href="#">Get Directions</a>
					  	</section>
					</div>
					<div class="col-md-8 row">
					  	<h2>Conveniently located near Newport News Williamsburg Intl. Airport</h4>
					  	<section class="col-md-6">
					  	<h5>Open daily from 6am to 10pm</h5>
					  	<p><strong>Airport Storage & Garages</strong><br>
					  	599 Choptank Lane<br>
					  	Newport News, VA 23602</p>
					  	<a class="btn-default btn" href="#">Call (757) 881-9204</a>
					  	</section>
					  	<section class="col-md-6">
					  	<p><strong>Office Hours</strong><br> 
					  	Mon-Friday 8 am to 5 pm<br>
					  	Saturday 8 am to 4 pm<br>
					  	Sunday Closed</p> 
					  	<a class="btn-default btn" href="#">Contact by Email</a>
					  	</section>
					</div>
				</div>
			</aside>
			<aside class="secondary row">
				  	<section class="col-md-4">
					  	<h3>Storage Units</h3>
					  	<p>Large storage units with 14’ ceilings and 10’ doors.  Contact us today to reserve your unit.</p>
					  	<table class="table">
						  	<caption>Limited Availability</caption>
						  	<tbody>
							  	<tr>
							  	<td>12 x 15</td>
							  	<td align="right">$175/mo.</td>
							  	</tr>
						  	</tbody>
					  	</table>
					</section>
					<section class="col-md-4">
					  	<h3>Garages</h3>
					  	<p>Large storage garages ideal for long term car storage, boat and trailer storage, RV storage, motorcycle storage. 14’ ceilings and 10’ doors for all units and garages.  <a href="#">Contact us today</a> to reserve your unit.</p>
					  	<table class="table">
						  	<caption>Now Available</caption>
						  	<tbody>
							  	<tr>
							  	<td>12 x 30</td>
							  	<td align="right">$250/mo.</td>
							  	</tr>
						  	</tbody>
					  	</table>

					</section>
					<section class="col-md-4">
					  	<h3>Facility Map</h3>
					  	<img  alt="map of storage facility">
					</section>
			</aside>		
		</main>
		<footer>
			<div class="row">
				<section class="col-sm-4">
					<p><strong>Airport Storage & Garages</strong><br>
						599 Choptank Lane<br>
						Newport News, VA 23602<br>
						(757) 881-9204</p>
				</section>
				<section class="col-sm-4">
					<p>OPEN DAILY from 6am to 10pm</p>
					<p>Office Hours<br> 
					Monday - Friday	8 am  to 5 pm<br>
					Saturday 8 am to 4pm<br>
					Sunday Closed</p>
				</section>
				<section class="col-sm-4">
					Section 3
				</section>
			</div><!-- /.wrap -->
		</footer>
	</div><!-- /.container -->
	<!--bower:js -->
	<script src="src/lib/bower/jquery/dist/jquery.js"></script>
	<script src="src/lib/bower/bootstrap-sass/assets/javascripts/bootstrap.js"></script>
	<!--endbower-->
	<script src="assets/js/scripts.js"></script>
</body>
</html>